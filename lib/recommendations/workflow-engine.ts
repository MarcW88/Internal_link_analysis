...
export type Prepared = {
  ...
  cannibalization: { type: string; label: string; urls: string[]; score: number }[];
};
...
export async function prepareCrawl(inlinkRows: CsvRow[], crawlRows: CsvRow[], targets?: string[], mappingRows?: MappingRow[]): Promise<Prepared> {
  ...
  const summary = {
    ...
    cannibalization: keywordCannibals.length + semanticCannibals.length,
  ...
  const pages = Object.fromEntries(pageAnalysis.entries());
  const targetsByAnchorRecord = Object.fromEntries([...targetsByAnchor.entries()].map(([k, v]) => [k, [...v]]));

  return {
    summary,
    raw: rawRecommendations,
    pages,
    targetsByAnchor: targetsByAnchorRecord,
    conflicts,
    cannibalization,
  };
}

export async function enrichRecommendation(rec: RawRec, pages: Record<string, Page>, targetsByAnchor: Record<string, string[]>, jinaTimeout = 45_000): Promise<Enriched> {
  const target = pages[rec.target];
  if (!target) {
    return { error: "Cible introuvable" };
  }

  try {
    const blocks = extractContentBlocks(await readWithJina(rec.source, jinaTimeout));
    ...
    if (selected) {
      ...
      const targetsForAnchor = targetsByAnchor[normalized];
      const conflict = Boolean(targetsForAnchor && targetsForAnchor.some((url) => url !== rec.target));
      if (conflict) return { scraped: true, fallback: true };
    ...
    return {
      recommendation: fallbackAnchor
        ? {
            source: rec.source,
            target: rec.target,
            anchor: fallbackAnchor,
            passage: rec.reasons.join(" | "),
            score: rec.score,
            direction: rec.direction,
            conflict: false,
            type: rec.type,
    ...
    return {
      recommendation: fallbackAnchor
        ...
        : undefined,
      fallback: true,
      error: error instanceof Error ? error.message : "Scraping impossible",
    };
  }
}

export function buildResult(prepared: Prepared, enriched: Enriched[]) {
  const recommendations = enriched
    .map((e) => e.recommendation)
    .filter((r): r is NonNullable<typeof r> => Boolean(r))
    .sort((a, b) => b.score - a.score);

  const scrapedPages = enriched.filter((e) => e.scraped).length;
  const scrapeFailures = enriched.filter((e) => e.error).length;
  const passagesWithoutAnchor = enriched.filter((e) => e.fallback && !e.error).length;
  const scrapeErrors = [...new Set(enriched.map((e) => e.error).filter((e): e is string => Boolean(e)))];
...
export async function analyzeCrawl(inlinkRows: CsvRow[], crawlRows: CsvRow[], targets?: string[], mappingRows?: MappingRow[]) {
  const prepared = await prepareCrawl(inlinkRows, crawlRows, targets, mappingRows);
  const top = prepared.raw.slice(0, targets?.length ? 12 : 10);
  const enriched = await Promise.all(top.map((rec) => enrichRecommendation(rec, prepared.pages, prepared.targetsByAnchor)));
  return buildResult(prepared, enriched);
}
